...
@st.cache
def load_data():
    df = pd.read_excel("/content/drive/MyDrive/Colab Notebooks/EVASAO/Relatorio-dados.xls")
    ...
    # Create a new 'Data' column with datetime values of 'Data do Último Procedimento' and NaN for '-'
    df_tec_integrado['Data'] = pd.to_datetime(df_tec_integrado['Data do Último Procedimento'], errors='coerce')

    # Create the 'ano_evasao' attribute from 'Data' column
    df_tec_integrado['ano_evasao'] = np.where(df_tec_integrado['Situação no Curso'] == 'Evasão', df_tec_integrado['Data'].dt.year, np.nan)
